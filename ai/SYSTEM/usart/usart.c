...
void USART1_IRQHandler(void)
{
	...
	{	 
		...
		if((USART1_RX_STA&0x8000)==0)//接收未完成
		{
			...
			{
				if(res!=0x0a)USART1_RX_STA=0;//接收错误,重新开始	  //0x0a为换行符
				else USART1_RX_STA|=0x8000;	//接收完成了 
			}else //还没收到0X0D
			{	
				if(res==0x0d)USART1_RX_STA|=0x4000;	   //0x0d为回车符
				else
				{
...
//CHECK OK
//091209
void uart_init(u32 pclk2, u32 pclk1, u32 bound)
{  	 
	float temp;
	u16 mantissa;
	u16 fraction;	   
...
//	USART1->CR1|=1<<8;    //PE中断使能
	...
	GPIOA->CRL|=0X00008B00;//IO状态设置 PA2设置为50MHz的复用功能推挽输出模式 PA3为上拉输入模式，不应该选为浮空输入
...
//	USART2->CR1|=1<<8;    //PE中断使能
	...
	MY_NVIC_Init(3,3,USART2_IRQChannel,1);//组2，最低优先级 
#endif
